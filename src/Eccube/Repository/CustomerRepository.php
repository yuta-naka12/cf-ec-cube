...
class CustomerRepository extends AbstractRepository
{
    ...
    public function getQueryBuilderBySearchData($searchData)
    {
        ...
        if (isset($searchData['multi']) && StringUtil::isNotBlank($searchData['multi'])) {
            ...
            $qb
            ->andWhere('c.id = :customer_id OR CONCAT(c.name01, c.name02) LIKE :name OR CONCAT(c.kana01, c.kana02) LIKE :kana OR c.email LIKE :email OR c.phone_number LIKE :phone_number')
            ->setParameter('customer_id', $id)
            ->setParameter('name', '%'.$clean_key_multi.'%')
            ->setParameter('kana', '%'.$clean_key_multi.'%')
            ->setParameter('email', '%'.$clean_key_multi.'%')
            ->setParameter('phone_number', '%'.$clean_key_multi.'%');
        }
        ...
        // Pref
        if (!empty($searchData['CourseMaster']) && $searchData['CourseMaster']) {
            $qb
                ->andWhere('c.CourseMaster = :CourseMaster')
                ->setParameter('CourseMaster', $searchData['CourseMaster']->getId());
        }
        ...
        // delivery_date
        if (isset($searchData['delivery_date']) && StringUtil::isNotBlank($searchData['delivery_date'])) {
            $qb
                ->andWhere('c.delivery_date LIKE :delivery_date')
                ->setParameter('delivery_date', '%'.$searchData['delivery_date'].'%');
        }

        // delivery_code_name
        if (isset($searchData['delivery_code_name']) && StringUtil::isNotBlank($searchData['delivery_code_name'])) {
            $qb
                ->andWhere('c.delivery_code_name LIKE :delivery_code_name')
                ->setParameter('delivery_code_name', '%'.$searchData['delivery_code_name'].'%');
        }

        // delivery_week
        if (isset($searchData['delivery_week']) && StringUtil::isNotBlank($searchData['delivery_week'])) {
            $qb
                ->andWhere('c.delivery_week_1 LIKE :delivery_week_1')
                ->setParameter('delivery_week_1', '%'.$searchData['delivery_week'].'%');

            $qb
                ->orWhere('c.delivery_week_2 LIKE :delivery_week_2')
                ->setParameter('delivery_week_2', '%'.$searchData['delivery_week'].'%');
        }
     ...
     * @return Customer|null 見つからない場合はnullを返す.
     */
    public function getRegularCustomerByEmailAndPhoneNumber($email, $phone_number)
    {
        return $this->findOneBy([
            'email' => $email,
     ...
     * @return bool
     */
    public function isExistPhoneNumber(string $phoneNumber , int $id): bool
    {
        $qb = $this->createQueryBuilder('c')
            ->andWhere('c.phone_number = :phone_number')
            ->andWhere('c.id != :id')
