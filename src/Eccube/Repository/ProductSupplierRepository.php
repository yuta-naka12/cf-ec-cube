...
class ProductSupplierRepository extends AbstractRepository
{
     ...
     */
    public function up(ProductSupplier $ProductSupplier)
    {
        $sortNo = $ProductSupplier->getSortNo();
        $ProductSupplier2 = $this->findOneBy(['sort_no' => $sortNo + 1]);

        if (!$ProductSupplier2) {
            throw new \Exception(sprintf('%s より上位のメーカーが存在しません.', $ProductSupplier->getId()));
        }

        $ProductSupplier->setSortNo($sortNo + 1);
        $ProductSupplier2->setSortNo($sortNo);

        $em = $this->getEntityManager();
        $em->persist($ProductSupplier);
        $em->persist($ProductSupplier2);
        $em->flush();
    }
     ...
     */
    public function down(ProductSupplier $ProductSupplier)
    {
        $sortNo = $ProductSupplier->getSortNo();
        $ProductSupplier2 = $this->findOneBy(['sort_no' => $sortNo - 1]);

        if (!$ProductSupplier2) {
            throw new \Exception(sprintf('%s より下位のメーカーが存在しません.', $ProductSupplier->getId()));
        }
    ...
    public function delete($ProductSupplier)
    {
        $this->createQueryBuilder('c')
            ...
            ->setParameter('sort_no', $ProductSupplier->getSortNo())
            ->getQuery()
            ->execute();
