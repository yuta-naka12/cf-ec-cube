...
use Customize\Repository\Admin\Master\DeliveryTypeRepository;
...
class EditController extends AbstractController
{
    ...
    public function index(Request $request, $id = null, RouterInterface $router)
    {
        ...
        // OrderItemsを設定
        if ($request->get('order-form') !== null) {
            $orders = $request->get('order-form');
            ...
            switch ($request->get('mode')) {
                case 'register':
                    ...
                    if (!$flowResult->hasError() && $form->isValid()) {
                        ...
                        //配達種別をセット
                        if (!empty($request->get('order')['DeliveryType'])) {
                            $deliveryTypeId = $request->get('order')['DeliveryType'];
                            $DeliveryType = $this->deliveryTypeRepository->find($deliveryTypeId);
                            $TargetOrder->setDeliveryType($DeliveryType);

                        }

                        $SenderId = $TargetOrder->getSender()->getId();
                        
                        if ($SenderId) {
                            $Sender = $this->senderRepository->find($SenderId);
        ...
        // 送り主検索フォーム
        $builder = $this->formFactory
            ->createBuilder(SearchSenderType::class);
        $searchSenderModalForm = $builder->getForm();
        ...
        return [
            ...
            'searchSenderModalForm' => $searchSenderModalForm->createView(),
            'searchProductModalForm' => $searchProductModalForm->createView(),
            'Order' => $TargetOrder,
            'OrderItems' => $this->serializer->serialize($TargetOrder->getOrderItems()->toArray(), 'json', ['groups' => ['order_item', 'product_class']]),
            'id' => $id,
    ...
    /**
     * @Route("/%eccube_admin_route%/order/{id}/edit/comment", requirements={"id" = "\d+"}, name="admin_order_edit_comment", methods={"GET", "POST"})
     * Method({"GET", "POST"})
     */
     ...
     * 送り主を検索する.
     *
     * @Route("/%eccube_admin_route%/order/search/sender/html", name="admin_order_search_sender_html", methods={"GET", "POST"})
     * @Route("/%eccube_admin_route%/order/search/sender/html/page/{page_no}", requirements={"page_no" = "\d+"}, name="admin_order_search_sender_html_page", methods={"GET", "POST"})
     * @Template("@admin/Order/search_sender.twig")
     *
     * @param Request $request
     * @param integer $page_no
     *
     * @return array
     */
    public function searchSenderHtml(Request $request, $page_no = null, PaginatorInterface $paginator)
    {
        if ($request->isXmlHttpRequest() && $this->isTokenValid()) {
            log_debug('search sender start.');
            $page_count = $this->eccubeConfig['eccube_default_page_count'];
            $session = $this->session;
            ...
            $qb = $this->senderRepository->getQueryBuilderBySearchData($searchData);
            /** @var \Knp\Component\Pager\Pagination\SlidingPagination $pagination */
            $pagination = $paginator->paginate(
                $qb,
                $page_no,
            ...
            /** @var $Senders \Customize\Entity\Order\Customer[] */
            $Senders = $pagination->getItems();
            ...
            foreach ($Senders as $Sender) {
                $data[] = [
                    'id' => $Sender->getId(),
                    'name' => sprintf($formatName, $Sender->getName01(), $Sender->getName02(),
                        $Sender->getKana01(),
                        $Sender->getKana02()),
                    'phone_number' => $Sender->getPhoneNumber(),
                    'company_name' => $Sender->getCompanyName(),
                ];
            }
     ...
     * 送り主情報を検索する.
     *
     * @Route("/%eccube_admin_route%/order/search/sender/id", name="admin_order_search_sender_by_id", methods={"POST"})
     *
     * @param Request $request
     *
     * @return \Symfony\Component\HttpFoundation\JsonResponse
     */
    public function searchSenderById(Request $request)
    {
        if ($request->isXmlHttpRequest() && $this->isTokenValid()) {
            log_debug('search sender by id start.');

            /** @var $Customer \Customize\Entity\Order\Sender */
            $Sender = $this->senderRepository
                ->find($request->get('id'));
            if (is_null($Sender)) {
                log_debug('search sender by id not found.');

                return $this->json([], 404);
            }

            log_debug('search sender by id found.');

            $data = [
                'id' => $Sender->getId(),
                'name01' => $Sender->getName01(),
                'name02' => $Sender->getName02(),
                'kana01' => $Sender->getKana01(),
                'kana02' => $Sender->getKana02(),
                'postal_code' => $Sender->getPostalCode(),
                'pref' => is_null($Sender->getPref()) ? null : $Sender->getPref()->getId(),
                'addr01' => $Sender->getAddr01(),
                'addr02' => $Sender->getAddr02(),
                'phone_number' => $Sender->getPhoneNumber(),
                'company_name' => $Sender->getCompanyName(),
    ...
    /**
     * @Route("/%eccube_admin_route%/api/order/search/product", name="api_admin_order_search_product", methods={"POST"})
     */
    public function searchProductApi(Request $request, $page_no = null, PaginatorInterface $paginator)
    {
        if ($request->isXmlHttpRequest() && $this->isTokenValid()) {
            log_debug('search product start.');
            $session = $this->session;
            ...
            $Products = $this->productRepository
                ->getQueryBuilderBySearchDataByArrayData($searchData);
    ...
    /**
     * @Route("/%eccube_admin_route%/order/search/product/data", name="admin_order_search_product_data", methods={"POST"})
     * @Route("/%eccube_admin_route%/order/search/product/page/{page_no}", requirements={"page_no" = "\d+"}, name="admin_order_search_product_page", methods={"GET", "POST"})
     */
    public function searchProductDataByCode(Request $request, PaginatorInterface $paginator)
    {
        if ($request->isXmlHttpRequest() && $this->isTokenValid()) {
            $searchData = [
            ...
            /** @var $Products \Eccube\Entity\Product[] */
            $Products = $this->productRepository
                ->getQueryBuilderByCode($searchData);
